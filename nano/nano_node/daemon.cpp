			...
			if (!node->init_error ())
			{
				...
				if (config.rpc_enable)
				{
					if (!config.rpc.child_process.enable)
					{
						...
						rpc_handler = std::make_unique<nano::inprocess_rpc_handler> (*node, ipc_server, config.rpc, [&ipc_server, &workers = node->workers, &io_ctx = node->io_ctx] () {
							ipc_server.stop ();
				...
				runner = std::make_unique<nano::thread_runner> (node->io_ctx, node->config.io_threads);
				runner->join ();

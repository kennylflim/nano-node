...
namespace lmdb
{
	...
	TEST (mdb_block_store, upgrade_v16_v17)
	{
		...
		= builder
		  ...
		  auto code
		= [&block1, &block2, &block3] (auto confirmation_height, nano::block_hash const & expected_cemented_frontier) {
			  auto path (nano::unique_path ());
			  nano::mdb_val value;
			  {
				  nano::logger_mt logger;
	...
	TEST (mdb_block_store, upgrade_v17_v18)
	{
		...
		= builder
		  ...
		  .sign (key2.prv, key2.pub)
		  .work (*pool.generate (key2.pub))
		  .build () auto state_send_epoch_link
		= builder
		  .state ()
		  .account (key2.pub)
		  .previous (state_open.hash ())

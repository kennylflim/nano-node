...
TEST (ledger, fail_change_old)
{
	...
	auto block = builder
				 ...
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (*pool.generate (nano::dev::genesis->hash ()))
				 .build () auto result1 (ledger.process (transaction, block));
...
TEST (ledger, fail_change_fork)
{
	...
	auto block1 = builder
				  ...
				  .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				  .work (*pool.generate (nano::dev::genesis->hash ()))
				  .build () auto result1 (ledger.process (transaction, block1));
	ASSERT_EQ (nano::process_result::progress, result1.code);
	nano::keypair key2;
	auto block2 = builder
				  .change ()
				  ...
				  .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				  .work (*pool.generate (nano::dev::genesis->hash ()))
				  .build () auto result2 (ledger.process (transaction, block2));
...
TEST (ledger, fail_receive_gap_source)
{
	...
	auto block3 = builder
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build () auto result3 (ledger.process (transaction, block3));
	...
	auto block4 = builder
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3.hash ()))
				  .build () auto result4 (ledger.process (transaction, block4));
...
TEST (ledger, fail_receive_overreceive)
{
	...
	auto block2 = builder
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (key1.pub))
				  .build () auto result3 (ledger.process (transaction, block2));
	ASSERT_EQ (nano::process_result::progress, result3.code);
	auto block3 = builder
				  .receive ()
				  .previous (block2.hash ())
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block2.hash ()))
				  .build () auto result4 (ledger.process (transaction, block3));
...
TEST (ledger, fail_receive_fork_previous)
{
	...
	auto block5 = builder
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3.hash ()))
				  .build () auto result5 (ledger.process (transaction, block5));
...
TEST (ledger, fail_receive_received_source)
{
	...
	auto block6 = builder
				  ...
				  .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				  .work (*pool.generate (block2.hash ()))
				  .build () auto result6 (ledger.process (transaction, block6));
	...
	auto block7 = builder
				  ...
				  .sign (key1.prv, key1.pub)
				  .work (*pool.generate (block3.hash ()))
				  .build () auto result7 (ledger.process (transaction, block7));
...
TEST (ledger, send_open_receive_rollback)
{
	...
	auto send1 = builder
				 ...
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (*pool.generate (info1.head))
				 .build () auto return1 (ledger.process (transaction, send1));
	ASSERT_EQ (nano::process_result::progress, return1.code);
	auto send2 = builder
				 .send ()
				 .previous (send1.hash ())
				 ...
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (*pool.generate (send1.hash ()))
				 .build () auto return2 (ledger.process (transaction, send2));
	ASSERT_EQ (nano::process_result::progress, return2.code);
	nano::keypair key2;
	auto open = builder
				.open ()
				...
				.sign (key1.prv, key1.pub)
				.work (*pool.generate (key1.pub))
				.build () auto return4 (ledger.process (transaction, open));
	...
	auto receive = builder
				   ...
				   .sign (key1.prv, key1.pub)
				   .work (*pool.generate (open.hash ()))
				   .build () auto return5 (ledger.process (transaction, receive));
	...
	auto change1 = builder
				   ...
				   .build () auto return6 (ledger.process (transaction, change1));
...
TEST (ledger, successor_epoch)
{
	...
	= builder
	  ...
	  .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
	  .work (*pool.generate (open.hash ()))
	  .build () auto transaction (node1.store.tx_begin_write ());
	ASSERT_EQ (nano::process_result::progress, node1.ledger.process (transaction, send1).code);
